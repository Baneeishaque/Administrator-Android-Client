...
public class OverlappingCopy {
	...
	private static void transformRect(Rect source, Rect transformedSource, int deltaX, int deltaY) {
		transformedSource.set(deltaX < 0 ? source.right * -1 : source.left,
				deltaY < 0 ? source.bottom * -1 : source.top, deltaX < 0 ? source.left * -1 : source.right,
				deltaY < 0 ? source.top * -1 : source.bottom);
	}

	private static void copyTransformedRect(Rect stepSourceRect, Rect stepDestRect, int deltaX, int deltaY, Bitmap data,
			Canvas bitmapBackedCanvas, Paint paint) {
	...
	public static void Copy(Bitmap data, Canvas bitmapBackedCanvas, Paint paint, Rect source, int destX, int destY,
			ObjectPool<Rect> rectPool) {
		// android.util.Log.i("LBM","Copy "+source.toString()+" to
		// "+destX+","+destY);
		...
		// Determine coordinate transform so that dest rectangle is always down
		// and to the right.
		...
		for (int xStep = 0; !xStepDone; xStep++) {
			...
			for (int yStep = 0; !yStepDone; yStep++) {
				...
				// android.util.Log.i("LBM","Copy transformed
				// "+stepSourceRect.toString()+" "+deltaX+" "+deltaY);
