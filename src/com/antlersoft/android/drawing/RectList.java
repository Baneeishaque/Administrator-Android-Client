...
/**
 * A list of rectangular regions that together represent an area of interest.
 * Provides a set of operations that apply to the whole area, adding, changing
 * and mutating the rectangles in the list as required.
 * <p>
 * Invariants: None of the rectangles in the list overlap; no pair of rectangles
 * in the list together make a single rectangle (none share a complete side)
 * </p>
 * <p>
...
public class RectList {
	...
	enum OverlapType {
		NONE, SAME, CONTAINS, CONTAINED_BY, COALESCIBLE, PARTIAL
	}
	...
	static class NonOverlappingPortion {
		...
		OverlapType overlap(Rect r1, Rect r2) {
			...
			} else if ((common & (LEFT | RIGHT)) == (LEFT | RIGHT)
					&& (verticalOverlap || (adjacent & (TOP | BOTTOM)) != 0)) {
			...
			} else if ((common & (TOP | BOTTOM)) == (TOP | BOTTOM)
					&& (horizontalOverlap || (adjacent & (LEFT | RIGHT)) != 0)) {
				result = OverlapType.COALESCIBLE;
	...
	/**
	 * Change the rectangle of interest to include only those portions that fall
	 * inside bounds.
	 * 
	 * @param bounds
	 */
