...
class ScaleGestureDetector implements IBCScaleGestureDetector {
	 ...
	 * 
	 * @see
	 * com.antlersoft.android.bc.IBCScaleGestureDetector#onTouchEvent(android.
	 * view.MotionEvent)
	 */
		...
		if (!mGestureInProgress) {
			...
			case MotionEvent.ACTION_POINTER_UP:
				if (mSloppyGesture) {
					...
					int id = (((action
							& MotionEvent.ACTION_POINTER_ID_MASK) >> MotionEvent.ACTION_POINTER_ID_SHIFT) == 0) ? 1 : 0;
			...
			case MotionEvent.ACTION_POINTER_UP:
				...
				// Set focus point to the remaining finger
				int id = (((action & MotionEvent.ACTION_POINTER_ID_MASK) >> MotionEvent.ACTION_POINTER_ID_SHIFT) == 0)
						? 1 : 0;
				mFocusX = event.getX(id);
				mFocusY = event.getY(id);
