...
class MetaKeyDialog extends Dialog implements ConnectionSettable {
		...
		menu.findItem(R.id.itemDeleteKeyList).setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {
			...
			public boolean onMenuItemClick(MenuItem item) {
				Utils.showYesNoPrompt(_canvasActivity, "Delete key list",
						"Delete list " + _textListName.getText().toString(), new OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int which) {
								...
								if (_listId > 1) {
									...
									db.execSQL(MessageFormat.format("DELETE FROM {0} WHERE {1} = {2}",
											MetaKeyBean.GEN_TABLE_NAME, MetaKeyBean.GEN_FIELD_METALISTID, _listId));
									db.execSQL(MessageFormat.format("DELETE FROM {0} WHERE {1} = {2}",
											MetaList.GEN_TABLE_NAME, MetaList.GEN_FIELD__ID, _listId));
		...
		menu.findItem(R.id.itemDeleteKey).setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {
			...
			public boolean onMenuItemClick(MenuItem item) {
				...
				if (position != Spinner.INVALID_POSITION) {
					...
					Utils.showYesNoPrompt(_canvasActivity, "Delete from list", "Delete key " + toDelete.getKeyDesc(),
							new OnClickListener() {
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		menu.findItem(R.id.itemDeleteKey)
				.setEnabled(_spinnerKeysInList.getSelectedItemPosition() != Spinner.INVALID_POSITION);
		return true;
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		_spinnerKeySelect.setAdapter(new ArrayAdapter<String>(getOwnerActivity(), android.R.layout.simple_spinner_item,
				MetaKeyBean.allKeysNames));
	...
	private String getCopyListString() {
		if (copyListString == null) {
			...
			for (Entry<String, Object> s : _currentKeyBean.Gen_getValues().valueSet()) {
				if (!s.getKey().equals(MetaKeyBean.GEN_FIELD__ID)
						&& !s.getKey().equals(MetaKeyBean.GEN_FIELD_METALISTID)) {
					fieldList.append(',');
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		if (!_justStarted && keyCode != KeyEvent.KEYCODE_BACK && keyCode != KeyEvent.KEYCODE_MENU
		...
		if (listId != _listId) {
			for (int i = 0; i < _lists.size(); ++i) {
				...
				if (list.get_Id() == listId) {
					...
					Cursor c = _database.getReadableDatabase()
							.rawQuery(
									MessageFormat.format("SELECT * FROM {0} WHERE {1} = {2} ORDER BY KEYDESC",
											MetaKeyBean.GEN_TABLE_NAME, MetaKeyBean.GEN_FIELD_METALISTID, listId),
									EMPTY_ARGS);
					...
					_spinnerKeysInList.setAdapter(
							new ArrayAdapter<String>(getOwnerActivity(), android.R.layout.simple_spinner_item, keys));
					if (keys.size() > 0) {
	...
	void setListSpinner() {
		...
		_spinnerLists.setAdapter(
				new ArrayAdapter<String>(getOwnerActivity(), android.R.layout.simple_spinner_item, listNames));
	}
