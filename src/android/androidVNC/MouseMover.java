...
class MouseMover extends Panner {
	...
	public void run() {
		...
		// Log.v(TAG, String.format("panning %f %d %d", scale,
		// (int)((double)velocity.x * scale), (int)((double)velocity.y *
		// scale)));
		if (canvas.processPointerEvent((int) (canvas.mouseX + ((double) velocity.x * scale)),
				(int) (canvas.mouseY + ((double) velocity.y * scale)), MotionEvent.ACTION_MOVE, 0, false, false)) {
