...
/**
 * Input handlers delegate to this class to handle keystrokes; this detects
 * keystrokes from the DPad and uses them to perform mouse actions; other
 * keystrokes are passed to VncCanvasActivity.defaultKeyXXXHandler
 * 
...
class DPadMouseKeyHandler {
	...
	public boolean onKeyDown(int keyCode, KeyEvent evt) {
		...
		case KeyEvent.KEYCODE_DPAD_CENTER:
			if (!mouseDown) {
				mouseDown = true;
				result = canvas.processPointerEvent(canvas.mouseX, canvas.mouseY, MotionEvent.ACTION_DOWN,
						evt.getMetaState(), mouseDown, canvas.cameraButtonDown);
			}
			break;
		default:
		...
		if ((xv != 0 || yv != 0) && !isMoving) {
			...
			canvas.processPointerEvent(canvas.mouseX + x, canvas.mouseY + y, MotionEvent.ACTION_MOVE,
					evt.getMetaState(), mouseDown, canvas.cameraButtonDown);
	...
	public boolean onKeyUp(int keyCode, KeyEvent evt) {
		...
		case KeyEvent.KEYCODE_DPAD_CENTER:
			if (mouseDown) {
				mouseDown = false;
				result = canvas.processPointerEvent(canvas.mouseX, canvas.mouseY, MotionEvent.ACTION_UP,
						evt.getMetaState(), mouseDown, canvas.cameraButtonDown);
			} else {
				result = true;
			}
