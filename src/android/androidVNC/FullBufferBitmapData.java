	...
	class Drawable extends AbstractBitmapDrawable {
		...
		public void draw(Canvas canvas) {
			if (vncCanvas.getScaleType() == ImageView.ScaleType.FIT_CENTER) {
				canvas.drawBitmap(data.bitmapPixels, 0, data.framebufferwidth, xoffset, yoffset, framebufferwidth,
						framebufferheight, false, null);
			} else {
				...
				canvas.drawBitmap(data.bitmapPixels, offset(xo, yo), data.framebufferwidth, xo, yo, drawWidth,
						drawHeight, false, null);
				/*
				 * } else { int scalewidth = (int)(vncCanvas.getVisibleWidth() /
				 * scale + 1); if (scalewidth + xo > data.framebufferwidth)
				 * scalewidth = data.framebufferwidth - xo; int scaleheight =
				 * (int)(vncCanvas.getVisibleHeight() / scale + 1); if
				 * (scaleheight + yo > data.framebufferheight) scaleheight =
				 * data.framebufferheight - yo;
				 * canvas.drawBitmap(data.bitmapPixels, offset(xo, yo),
				 * data.framebufferwidth, xo, yo, scalewidth, scaleheight,
				 * false, null); }
				 */
			}
