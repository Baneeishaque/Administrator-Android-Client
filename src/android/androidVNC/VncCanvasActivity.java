...
public class VncCanvasActivity extends Activity {
	...
	class ZoomInputHandler extends AbstractGestureInputHandler {
		 ...
		 * 
		 * @see
		 * android.androidVNC.AbstractInputHandler#onTrackballEvent(android.view
		 * .MotionEvent)
		 */
		@Override
	...
	private static final int inputModeIds[] = { R.id.itemInputFitToScreen, R.id.itemInputTouchpad, R.id.itemInputMouse,
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		menu.findItem(R.id.itemSpecialKeys).setVisible(false);
		menu.findItem(R.id.itemCenterMouse).setVisible(false);
		menu.findItem(R.id.itemColorMode).setVisible(false);
		menu.findItem(R.id.itemScaling).setVisible(false);
		menu.findItem(R.id.itemCtrlAltDel).setVisible(false);
		menu.findItem(R.id.itemInfo).setVisible(false);
		menu.findItem(R.id.itemArrowDown).setVisible(false);
		menu.findItem(R.id.itemArrowUp).setVisible(false);
		menu.findItem(R.id.itemArrowLeft).setVisible(false);
		menu.findItem(R.id.itemArrowRight).setVisible(false);
		menu.findItem(R.id.itemOpenDoc).setVisible(false);
		menu.findItem(R.id.itemSendKeyAgain).setVisible(false);
		menu.findItem(R.id.itemInputMode).setVisible(false);
		menu.findItem(R.id.itemFollowMouse).setVisible(false);
		menu.findItem(R.id.itemFollowPan).setVisible(false);
	...
	private void sendSpecialKeyAgain() {
		if (lastSentKey == null || lastSentKey.get_Id() != connection.getLastMetaKeyId()) {
			...
			Cursor c = database.getReadableDatabase()
					.rawQuery(
							MessageFormat.format("SELECT * FROM {0} WHERE {1} = {2}", MetaKeyBean.GEN_TABLE_NAME,
									MetaKeyBean.GEN_FIELD__ID, connection.getLastMetaKeyId()),
