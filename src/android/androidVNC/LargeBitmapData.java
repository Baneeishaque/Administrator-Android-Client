...
class LargeBitmapData extends AbstractBitmapData {
	...
	/**
	 * Multiply this times total number of pixels to get estimate of process
	 * size with all buffers plus safety factor
	 */
	...
	/**
	 * Pool of temporary rectangle objects. Need to synchronize externally
	...
	class LargeBitmapDrawable extends AbstractBitmapDrawable {
		 ...
		 * 
		 * @see
		 * android.graphics.drawable.DrawableContainer#draw(android.graphics.
		 * Canvas)
		 */
		@Override
	 ...
	 * @param c
	 *            View that will display screen
	 * @param displayWidth
	 * @param displayHeight
	 * @param capacity
	 *            Max process heap size in bytes
	 */
	 ...
	 * 
	 * @see
	 * android.androidVNC.AbstractBitmapData#copyRect(android.graphics.Rect,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see android.androidVNC.AbstractBitmapData#drawRect(int, int, int, int,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see android.androidVNC.AbstractBitmapData#updateBitmap(int, int, int,
	 * int)
	 */
	@Override
	...
	synchronized boolean validDraw(int x, int y, int w, int h) {
		// android.util.Log.i("LBM", "Validate Drawing "+x+" "+y+" "+w+" "+h+"
		// "+xoffset+" "+yoffset+" "+(x-xoffset>=0 && x-xoffset+w<=bitmapwidth
		// && y-yoffset>=0 && y-yoffset+h<=bitmapheight));
		boolean result = x - xoffset >= 0 && x - xoffset + w <= bitmapwidth && y - yoffset >= 0
				&& y - yoffset + h <= bitmapheight;
	 ...
	 * 
	 * @see
	 * android.androidVNC.AbstractBitmapData#writeFullUpdateRequest(boolean)
	 */
	@Override
	...
	synchronized void syncScroll() {
		...
		if (deltaX != 0 || deltaY != 0) {
			...
			if (Math.abs(deltaX) < bitmapwidth && Math.abs(deltaY) < bitmapheight) {
				...
				try {
					...
					sourceRect.set(deltaX < 0 ? -deltaX : 0, deltaY < 0 ? -deltaY : 0,
							deltaX < 0 ? bitmapwidth : bitmapwidth - deltaX,
							deltaY < 0 ? bitmapheight : bitmapheight - deltaY);
					if (!invalidList.testIntersect(sourceRect)) {
						didOverlapping = true;
						OverlappingCopy.Copy(mbitmap, memGraphics, defaultPaint, sourceRect, deltaX + sourceRect.left,
								deltaY + sourceRect.top, rectPool);
			...
			if (!didOverlapping) {
				try {
					// android.util.Log.i("LBM","update req "+xoffset+"
					// "+yoffset);
		...
		for (int i = 0; i < size; i++) {
			...
			try {
				rfb.writeFramebufferUpdateRequest(invalidRect.left, invalidRect.top,
						invalidRect.right - invalidRect.left, invalidRect.bottom - invalidRect.top, false);
		...
		// android.util.Log.i("LBM", "pending "+pendingList.toString() +
		// "invalid "+invalidList.toString());
	}
}
